...
class ShopkeeperController {
    async createShop(req, res) {
        const { shopName, shopDescription, website, companyUEN, userid } = req.body;
        if (!shopName || !shopDescription || !userid) return res.status(404).json({ message: 'Please fill all the fields' });
        const shop = await shopService.findShop({ userid: userid });
        if (shop) {
        ...
        let currshop;
        try {
            currshop = await shopService.createShop({
                shopName: shopName,
            ...
            return res.status(200).json({ message: 'Shop created successfully', shop: new shopDto(currshop) });
        }
        catch (error) {
    ...
    async getInventory(req, res) {
        ...
        try {
            ...
            return res.status(500).json({status:false,message:'something went wrong while fetching inventory'})
        }
    }   
